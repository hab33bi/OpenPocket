...
/// Water app tables (docs/water/IMPL-SPEC.md): the deep-indigo→neon-blue→
/// cyan→white colour gradient (RGB565-BE pairs, stamped raw at blit time —
/// no per-pixel divide), the PUSH_LUT[d2] repulsion table (bakes
/// STIFF·(H−d)/d so the hot pair loop is sqrt- AND divide-free), and two
/// glow colour tables (tint·v → RGB565-BE) so the surface sparkle + meniscus
/// line MAX-blend with zero divides.
///
/// Review-fix: PUSH_LUT is clamped to i16::MAX (not 8192) — the 8192 clamp
/// made the kernel NON-MONOTONIC below ~2.6 px (an inverted spring) that
/// collapsed particles to a co-located singularity. STIFF·(H−d)/d ≤ 24000 <
/// 32767 for every d2≥1, so unclamped the impulse is bounded and monotonic
/// (strongest at contact) — the anti-singularity guarantee.
fn generate_water_lut() {
    let to565 = |r: i32, g: i32, b: i32| -> u16 {
    ...
    // WATER_LUT — 256-entry gradient across 5 stops.
    ...
    for i in 0..256i32 {
        ...
        let px = to565(lerp(r0, r1, t, d), lerp(g0, g1, t, d), lerp(b0, b1, t, d));
        if i > 0 {
            body.push(',');
        }
    ...
    // PUSH_LUT[d2] — repulsion, monotonic (clamp = i16::MAX). STIFF=1600.
    const HP: f64 = 16.0;
    ...
    for d2 in 0..=256usize {
        ...
        body.push_str(&format!("{}", (v as i32).clamp(0, 32767)));
    }
    body.push_str("];\n");

    // Glow colour tables (tint·v → RGB565-BE) — divide-free MAX-blend glow.
    for (name, (tr, tg, tb)) in [
        ("GLOW_SPARK", (0i32, 210i32, 255i32)),
        ("GLOW_LINE", (150i32, 240i32, 255i32)),
    ] {
        body.push_str(&format!("pub static {name}: [(u8, u8); 256] = ["));
        for v in 0..256i32 {
            let px = to565(tr * v / 255, tg * v / 255, tb * v / 255);
            if v > 0 {
                body.push(',');
            }
            body.push_str(&format!("({},{})", (px >> 8) as u8, px as u8));
