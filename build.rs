...
fn generate_wheel_assets() {
    ...
    const ICON_H: u32 = 96; // hero (open-morph flight target / app template)
    ...
    for (idx, (name, file)) in WHEEL_APPS.iter().enumerate() {
        ...
        for (tag, px) in [("S", ICON_S), ("L", ICON_L), ("H", ICON_H)] {
        ...
        table.push_str(&format!(
            "    WheelApp {{ name: {name:?}, icon_s: ICON_{idx}_S, icon_l: ICON_{idx}_L, icon_h: ICON_{idx}_H }},\n"
        ));
        println!("cargo:rerun-if-changed={path}");
    }
    table.push_str("];\n");
    body.push_str(
        "#[derive(Clone, Copy)]\npub struct WheelApp { pub name: &'static str, pub icon_s: &'static [u8], pub icon_l: &'static [u8], pub icon_h: &'static [u8] }\n",
    );
    body.push_str(&table);
